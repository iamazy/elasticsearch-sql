 ...
 * @descrition
 **/
public class FuzzyLikeQueryParser extends AbstractExactQueryParser implements LikeQueryParser{
    @Override
    public AtomicQuery parse(ElasticsearchParser.LikeClauseContext expression) {
        ...
        return parseCondition(expression, operator, null, (fieldName, operator1, rightParams) -> {
            ...
            QueryBuilder fuzzyQueryBuilder = QueryBuilders.fuzzyQuery(fieldName, pattern);
            switch (operator1) {
                case Like: {
                ...
                default:
                case NotLike: {
                    return QueryBuilders.boolQuery().mustNot(fuzzyQueryBuilder);
                }
            }
        });
    }
