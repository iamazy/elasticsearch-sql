 ...
 * @date 2019/11/21
 **/
public class WildcardLikeQueryParser extends AbstractExactQueryParser implements LikeQueryParser {
    ...
    public AtomicQuery parse(ElasticsearchParser.LikeClauseContext expression) {
        ...
        return parseCondition(expression, operator, null, (fieldName, operator1, rightParams) -> {
            ...
            QueryBuilder wildcardQuery = QueryBuilders.wildcardQuery(fieldName, pattern);
            switch (operator1) {
                case Like: {
                ...
                default:
                case NotLike: {
                    return QueryBuilders.boolQuery().mustNot(wildcardQuery);
                }
            }
        });
    }
