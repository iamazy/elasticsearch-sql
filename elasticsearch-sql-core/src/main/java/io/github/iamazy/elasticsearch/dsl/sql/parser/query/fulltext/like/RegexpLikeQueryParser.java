 ...
 * @date 2019/11/21
 **/
public class RegexpLikeQueryParser extends AbstractExactQueryParser implements LikeQueryParser {
    ...
    public AtomicQuery parse(ElasticsearchParser.LikeClauseContext expression) {
        ...
        return parseCondition(expression, operator, null, (fieldName, operator1, rightParams) -> {
            ...
            QueryBuilder regexpQuery = QueryBuilders.regexpQuery(fieldName, pattern);
            switch (operator1) {
                case Like: {
                    return regexpQuery;
                }
                default:
                case NotLike: {
                    return QueryBuilders.boolQuery().mustNot(regexpQuery);
                }
            }
        });
    }
